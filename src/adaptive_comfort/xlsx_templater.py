...
def headertable_fromdict(header):
    header = pd.DataFrame.from_dict(header, orient="index").rename(
...
def generate_sheet_json(data_object, fpth):
    ...
    def add_defaults(di):
        req = {"xlsx_exporter": "sheet_table", "xlsx_params": None}
        li = list(req.keys())
        for l in li:
            if l not in di.keys():
                di[l] = req[l]
    ...
    if type(data_object) == list:
        ...
        for l in data_object:
            if type(l) == pd.DataFrame:
                ...
                print(
                    "you need to pass a list of dataframes or dicts for this function to work"
                )
    ...
    if col_formatting != None:
        for col in col_formatting:
            ...
            worksheet.set_column(
                col["start_col"], col["end_col"], col["col_width"], f, col["options"]
            )
...
def main(data_object, fpth, open=True, print_fpth=False, FileLink_fpth=True):
...
def to_excel(data_object, fpth, open=True, print_fpth=False, FileLink_fpth=True):
    """
    Example:
        di = {
...
if __name__ == "__main__":
    if __debug__ == True:
        wdir = os.path.join(
            os.environ["mf_root"], r"MF_Toolbox\dev\examples\xlsx_templater"
        )
