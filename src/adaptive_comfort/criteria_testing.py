...
def criterion_hours_of_exceedance(arr_deltaT_hourly, arr_occupancy_hourly):
    ...
    *See CIBSE TM52: 2013, Page 13, Section 6.1.2a*
    ...
    """
    arr_deltaT_may_to_sept_hourly = arr_deltaT_hourly[
        :, :, MAY_START_HOUR:SEPT_END_HOUR
    ]  # Obtaining deltaT between May and end of September
    arr_occupancy_may_to_sept_hourly = arr_occupancy_hourly[
        :, MAY_START_HOUR:SEPT_END_HOUR
    ]  # Obtaining occupancy between May and end of September

    arr_deltaT_occupied_may_to_sept_hourly = np.where(
        arr_occupancy_may_to_sept_hourly == 0, 0, arr_deltaT_may_to_sept_hourly
    ...
    arr_deltaT_occupied_may_to_sept_hourly = (
        arr_deltaT_occupied_may_to_sept_hourly.round()
    )

    arr_deltaT_bool = (
        arr_deltaT_occupied_may_to_sept_hourly >= 1
    )  # Find where delta T is greater than or equal to 1K.
    arr_room_total_hours_exceedance = arr_deltaT_bool.sum(
        axis=2
    )  # Sum along last axis (hours) to get total hours of exceedance per room

    arr_occupancy_bool = (
        arr_occupancy_may_to_sept_hourly > 0
    )  # True where hour has occupancy greater than 0
    arr_occupancy_3_percent = (
        arr_occupancy_bool.sum(axis=1) * 0.03
    )  # Sum along time-step axis (hours), i.e. sum hours per room where occupied

    arr_bool = arr_room_total_hours_exceedance > arr_occupancy_3_percent
    arr_percent = (
        arr_room_total_hours_exceedance / arr_occupancy_bool.sum(axis=1)
    ) * 100  # Percentage of occupied hours exceeded out of total occupied hours
...
def criterion_bedroom_comfort(arr_op_temp_v_hourly):
    ...
    if arr_op_temp_v_hourly.shape[2] != 8760:
        raise ValueError(
            "Reporting intervals are not hourly for the operative temperature."
        )
    ...
    arr_bool = (
        arr_bedroom_comfort_total_hours > 32
    )  # Can't exceed 32 hours (1 percent of annual hours between 10pm and 7am)
    arr_percent = (
        arr_bedroom_comfort_total_hours / arr_op_temp_v_bedroom_comfort.shape[2]
    ) * 100  # Percentage of hours exceeding 26 degrees celsius over total annual hours between 10pm and 7am
