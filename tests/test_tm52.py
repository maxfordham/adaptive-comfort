...
class TestCheckResults:
    ...
    def test_all_criteria(self):
        ...
        chararr_one = np.char.array(
            np.where(df_mf_v_0_1["Criterion 1 (Pass/Fail)"] == "Fail", "1", "")
        )
        chararr_two = np.char.array(
            np.where(df_mf_v_0_1["Criterion 2 (Pass/Fail)"] == "Fail", "2", "")
        )
        chararr_three = np.char.array(
            np.where(df_mf_v_0_1["Criterion 3 (Pass/Fail)"] == "Fail", "3", "")
        )
        ...
        for criterion, di_name in di_names.items():
            criterion_abs_change = (
                df_mf_v_0_1[di_name["mf_name"]] - df_ies_v_0_1[di_name["ies_name"]]
            )
    ...
    def test_operative_temp(self):
        ...
        for i, j in enumerate(sorted(di_op_temp.items())):
            ...
            abs_change = self.tm52_calc.arr_op_temp_v[0][i].round(
                3
            ) - arr_op_temp.astype("float64").round(3)
        ...
        abs_change = self.tm52_calc.arr_op_temp_v[0].round(3) - np.array(
            [j for i, j in sorted(di_op_temp.items())]
        ).round(3)
        rel_change = abs_change / (
            np.array([j for i, j in sorted(di_op_temp.items())]).round(3)
        )
        assert (abs_change <= 1).sum(dtype=bool)
        assert (rel_change < 5).sum(dtype=bool)
